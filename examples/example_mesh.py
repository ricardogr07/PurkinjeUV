...
class Mesh:
    ...
    def __init__(self, filename, useVTK=True):
        ...
        useVTK = useVTK and hasVTK

        if useVTK:
            # we use VTK to create a faster projector onto the mesh
            # using CellLocator
            #
            ...
            self.vtk_locator = locator
            self.project_new_point = self.vtk_project_new_point

        else:
            self.project_new_point = self.kdtree_project_new_point
    ...
    def vtk_project_new_point(self, point):
        "Project a point to the mesh (VTK version)"

        cellId = vtk.reference(0)
        c = [0.0, 0.0, 0.0]
        subId = vtk.reference(0)
        d = vtk.reference(0.0)
        self.vtk_locator.FindClosestPoint(point, c, cellId, subId, d)

        return c, int(cellId)

    def kdtree_project_new_point(self, point):
...
if __name__ == "__main__":
    m = Mesh("sphere.obj", useVTK=True)
    p = [0.14, 0.42, 0.9]
    print(m.project_new_point(p))
