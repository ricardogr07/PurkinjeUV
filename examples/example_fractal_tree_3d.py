import sys
import numpy as np
from purkinje_uv import Nodes, Branch
from random import shuffle
from example_mesh import Mesh
    ...
    for i in range(param.N_it):
        ...
        for g in branches_to_grow:
            ...
            for j in range(2):
                ...
                last_branch += 1
                # print(last_branch)
                length = param.length + np.random.normal(0, param.std_length)
                if length < param.min_length:
                    length = param.min_length
                branches[last_branch] = Branch(
                    m,
                    branches[g].nodes[-1],
                    branches[g].dir,
    ...
    if param.save:
        if param.save_paraview:
            ...
            for i in range(len(nodes.nodes)):
                xyz[i, :] = nodes.nodes[i]
            VTUWriter.write_line_vtu(xyz, ien, param.filename + ".vtu")
